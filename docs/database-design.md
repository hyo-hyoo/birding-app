...
> 建立日期：2026-08-17  
> 当前状态：完整设计基线未变；M1 与 M2-A 的五张账户表已实施，M2-B／C 的邮件流程、正式页面和维护清理已通过机械验证；阶段 6 待用户验收
...
以下是数据库设计必须支持的既有业务规则；设计见第10、12～15节。M1 与 M2-A～C 已实现并机械验证账户／Session、验证令牌、数据库限流、注册／邮件流程、正式账户页面和维护清理；阶段 6 仍待用户验收，密码辅助、观察与识别流程仍待后续批次。
...
M2-A 已验证令牌确认、账户无关限流入口、真实限流并发与 SQL 约束；M2-B 已验证入队失败保旧、孤立 Job、真实令牌切换、并发重发和 worker 状态重检；M2-C 已验证正式页面／邮件链路、两个重发入口、CSRF、受保护页面和维护清理。完整证据见后端计划 12.6～12.8 节。阶段 6 尚待用户验收，且这些结果不能外推为密码辅助、Observation、生产队列或生产邮件已经完成。
...
以上为完整 MVP 的设计和测试要求。M1 与 M2-A～C 已有认证、令牌确认、限流、邮件／日志隐私、正式页面与维护清理的本地机械验证证据；阶段 6 用户验收、密码辅助、观察业务和生产安全验证仍未完成，不能外推其覆盖范围。
...
阶段6的M1与M2-A五表已落地，迁移文件对应见第9节，未改变完整设计与原Migration批次；M2-B／C只是后端计划中的工程实施分组，没有新增表。正式“注册→验证→手动登录→空历史→退出”链路已通过机械验证，但仍须用户验收后才能完成阶段6。测试专用入口、令牌夹具或跳过验证的临时登录不能替代正式切片；密码重置仍延后。
...
- 阶段6仍需用户验收业务行为和中日文可见反馈；M3～M5继续逐批验证结构与业务约束，密码辅助还须回归14.5的登录竞态。
...
M1与M2-A～C已完成实现和机械验证，当前五张账户业务表不变。下一步是用户验收阶段6完整账户闭环与中日文页面反馈；验收通过后再按当前暂定顺序进入阶段7 Observation核心闭环。每批继续先审查最小改动及测试，不一次性创建其余表；阶段状态与验证计数只在后端计划维护。
