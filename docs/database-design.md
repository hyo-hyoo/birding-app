...
> 建立日期：2026-08-17  
> 当前状态：完整设计基线未变；M1 两表与 M2-A 验证令牌／限流三表已实施并验证，令牌签发与邮件投递、正式页面及维护清理尚未完成
...
以下是数据库设计必须支持的既有业务规则；设计见第10、12～15节。M1 与 M2-A 已实现账户／Session、令牌检查与确认、数据库限流基础，不代表下列全部流程已实现；注册、令牌签发与邮件、正式页面及后续业务仍待对应批次完成。
...
这些是完整 MVP 的设计约束。M1 与 M2-A 已验证当前五表的真实 SQL 非法写入拒绝及各自相关并发；M2-A 包含令牌确认原子性、限流键竞争、双维度计数和窗口边界。它们不能证明真实签发／重发、邮件 Job 或其余五表已经通过；M2 剩余门槛见 14.6 节，M3～M5 仍须逐批验证数据库保护。
...
实施边界：M1 已验证当前 Session 退出删除及到期立即不可认证；M2-A 已验证令牌状态判断和清理后找不到令牌时返回无效，但测试夹具清除不等于维护命令已经实现。过期 Session、失效验证令牌、限流尝试及孤立键的维护清理仍留 M2-C，在阶段 6 结束前落实；密码重置令牌清理随 M5 处理。
...
工程协议（M2-A 已验证独立限流计数及令牌确认基础；以下真实注册、签发／切换与邮件 Job 协作仍待 M2-B 以故障注入和真实线程验证）：
...
M2-A 已有令牌确认、账户无关的限流入口、真实限流并发与 SQL 约束证据；其中的并发确认不同于并发签发／重发。M2-B 仍须验证入队失败保旧、孤立 Job、真实令牌切换和 worker 状态重检；最终页面／邮件全链路的不泄露账户状态、两个正式重发入口串联及维护清理仍须随 M2-B／C 完成，不能只用测试令牌夹具或局部限流结果代替。
...
以上为完整 MVP 的设计和测试要求。M1 与 M2-A 已有认证、令牌确认、限流基础及局部日志防护证据；正式页面认证、邮件／日志隐私全链路、维护清理和生产安全验证仍未完成，不能外推其覆盖范围。
...
阶段6的M1与M2-A五表已落地，迁移文件对应见第9节，未改变完整设计与原Migration批次；M2-B／C只是后端计划中的暂定工程拆分。完成剩余注册、签发／邮件和正式页面串联后，才验收端到端“注册→验证→手动登录→空历史→退出”。不能用测试专用入口、令牌夹具或跳过验证的临时登录替代正式切片；密码重置仍延后。
...
- M2-A已验证三份新Migration、M2-only回滚／重建、五表schema.rb全新加载、令牌确认原子性、限流并发与局部日志防护；命令、结果和覆盖边界见后端计划12.6节。测试令牌夹具不是实际签发／重发流程。
- M2-B／C仍须完成14.6的剩余故障注入／并发及全链路检查、邮件日志防护和维护清理；不得把局部限流、令牌确认或test adapter成功入队记录视为完整M2通过。M3～M5继续逐批验证结构与业务约束，密码辅助还须回归14.5的登录竞态。
...
M1与M2-A已落地。按后端计划的暂定工程拆分，继续M2-B注册、令牌签发／切换及邮件Job，随后M2-C正式页面与维护清理；保留14.6完整故障验证门槛，不因三表已建立就将M2或阶段6标为完成。每批先审查最小改动及测试，不一次性创建其余表；阶段状态与验证计数只在后端计划维护。
