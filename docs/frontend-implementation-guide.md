...
> 当前适用阶段：静态前端 14／14 已验收；阶段 6 正式账户闭环已接入真实后端，并通过机械验证和用户验收；当前进入阶段 7 Observation 核心闭环<br>
...
当前 Rails 前端同时包含已验收的静态预览和阶段 6 的正式账户页面：
...
- 观察历史当前只有受保护的空状态，Observation 新建、观察数据、密码重置和修改密码仍未接入；
- 刷新页面会重置浏览器内的交互状态；
...
| 轮廓选择 | 原型使用示例分组、名称、顺序和图片 | 正式结构为水面／水边／大鸟／小鸟两层入口，并已确认 16 个具体轮廓和 1 个兜底选项 | 正式清单见 `requirements.md` 第 7 节；现有预览仍是 observed-only，不得新增第三层或把科名保存为鉴定结果 |
| 次要颜色 | 高保真示例没有完整展开所有次要颜色状态 | 每个部位提供可选次要颜色，并允许取消 | 已确认适配：正式需求包含多个显著颜色 |
| 颜色、特征与位置列表 | 原型包含演示名称和有限示例 | 当前各提供有限静态选项，文案明确为示例 | 产品清单已经确认并由 `requirements.md` 第 7 节拥有；当前 Rails 预览尚未同步，具体排版仍待 UI 实施 |
...
| 表单提交 | 原型点击可演示流程 | 静态预览仍标记 `data-static-preview="true"`；正式账户表单已有真实 action、Controller 和 CSRF 保护 | 已确认技术边界；精确路径、PRG 方式和错误微文案是当前工程实现，不是不可调整的产品／UI 决定 |
...
- 阶段 6 账户表单已接入真实校验、提交结果和服务器错误反馈，完整账户闭环及中日文可见反馈已经通过用户验收；阶段 7 Observation 真实表单、持久化与并发冲突反馈尚未实现；
- 当前 SVG 鸟图和缩略图只是概念性前端资产；
- 静态预览页面本身仍不提交真实业务数据；正式账户页面已经接入认证和账户持久化，但 Observation、密码辅助及其业务 Turbo 响应链路尚未完成；
