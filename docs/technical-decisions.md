...
Rails 初始化阶段的实际验证结果为：Rails 测试 1 个测试、1 个断言，0 失败、0 错误、0 跳过；System Test 当前 0 个测试；RuboCop 无违规；Brakeman 无安全警告。该结果只证明基础骨架与健康检查通过，不代表业务功能已经覆盖。
...
- `zh`、`zh-CN`、`zh-Hans` 等浏览器语言标记的最终归类规则；
