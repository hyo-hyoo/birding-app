...
> 当前适用阶段：静态前端 14／14、阶段 6 账户闭环与阶段 7 Observation 核心闭环均已验收；阶段 8A 视觉语法已确认，代表性试稿外轮廓来源冲突待确认<br>
...
- `anatidae`、`ardeidae`、`compact_passerine` 三个代表性试稿已经验证并确认上述视觉语法，但具体 SVG 路径、尺寸、示例配色和外轮廓来源未获确认；现有 `anatidae.svg`、`ardeidae.svg` 的飞行姿态与水面鸭、站立鹭试稿不一致，开发必须先按用户决定解决来源约束，不能直接复制试稿外形；
...
| 抽象鸟图 | 高保真基线展示概念性鸟图 | 当前 ERB Partial 和较早阶段 8A 渲染器仍含固定概念 SVG、密集纹理或多层特征模块；需改为 17 套四分区服务器生成映射 | 三个试稿只确认大色块、每部位最多一个连续补充色区等视觉语法；鸭科、鹭科外轮廓与登记源资产姿态不一致，需先解决来源约束，不能把旧实现或试稿路径当成正式资产 |
...
- 轮廓选择当前已有 17 张本地 PhyloPic CC0 SVG 及逐张许可记录；阶段 8A 三个代表性试稿的视觉语法已经确认，但当前 Rails 渲染器尚未完整落实，鸭科与鹭科试稿外形也与登记源资产姿态不一致；动态部位着色鸟图、摘要和历史缩略图仍未完成；
...
- 轮廓选择素材与阶段 8 生成映射必须区分：17 张轮廓选择 SVG 已落地并记录 CC0 来源；17 套四分区映射、结构特征局部变化和单一连续补充色区域已经确认。三个代表性试稿的视觉语法已获确认，但具体外轮廓来源合规未获确认，剩余矢量绘制仍未完成；
