...
> 建立日期：2026-08-17  
> 当前状态：完整设计基线未变；M1、M2 的五张账户表与 M3 的三张观察表已实施；阶段 7 工程实现、机械验证和用户验收完成；阶段 8 不改变数据库结构
...
物理依赖允许M4、M5独立推进，不把表编号误认必须线性。已确认的阶段 8 SVG／摘要增强不新增 Migration、数据库字段或表，SVG、摘要和缩略图继续从结构化数据派生。M3第一次可保存Observation必须已经有PartImpression，不能拆出“仅轮廓保存”中间交付。
...
阶段6的M1与M2-A五表已经落地；M2-B／C只是工程实施分组，没有新增表。阶段7的M3三表也已按第9节设计落地，并完成迁移往返、聚合事务、授权、并发验证和用户可见流程验收。阶段 8 不改变 Schema；M4／M5 尚未实施，密码重置仍延后。
...
- 阶段6业务行为和中日文可见反馈已通过用户验收；M3 已完成结构、聚合事务、授权、并发验证和阶段 7 用户可见流程验收；M4～M5 继续逐批验证，密码辅助还须回归14.5的登录竞态。
...
M1与M2-A～C已完成实现、机械验证和阶段6用户验收；M3 三张观察表及阶段 7B-1a～7E 的工程实现、机械验证和用户验收也已完成。当前共八张业务表；阶段 8 不新增数据库结构，M4 鸟种识别、M5 密码辅助和生产部署仍未实施。阶段状态与完整验证计数只在后端计划维护。
