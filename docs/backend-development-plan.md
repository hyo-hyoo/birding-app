...
| --- | --- | --- | --- | --- |
| 1 | 后端设计快速收口 | 正式需求、MVP、UI、技术决定 | 未开始 | `confirmed` |
...
- 密码重置与登录后修改密码；
- Observation 与个人数据授权；
- PartImpression 与结构化视觉记录；
- BirdCandidate 与最终鸟名；
- ActivityLocationSelection；
- 版本控制中的轮廓、颜色、特征和行动位置配置；
- 服务器生成的 SVG 与文字摘要；
- locale Cookie 与邮件 locale。
...
进入阶段 1「后端设计快速收口」，依次确认：

1. 业务模块边界；
2. 主要请求与数据流；
3. 当前用户授权边界；
4. 持久化、派生、配置与浏览器状态分类；
5. 进入 ER 图阶段前必须解决的未确认问题。
