...
> 建立日期：2026-08-17  
> 当前阶段：阶段 8 已完成工程实现、机械验证和用户验收；下一开发阶段尚未讨论或确认
...
阶段 6：纵向切片 1
  注册 → 验证 → 登录 → 空历史 → 退出
                    ↓
阶段 7：Observation 核心闭环（已完成）
                    ↓
阶段 8：SVG 与摘要渐进增强（已完成）
                    ↓
阶段 9：鸟种确认、账户辅助与设置（后续顺序暂定）
                    ↓
阶段 10：全 MVP 回归与本地验收
```
...
| 6 | 纵向切片 1：可登录的空观察历史 | 阶段 5 | 已完成：实现、机械验证与用户验收均通过 | `confirmed` |
| 7 | Observation 核心闭环 | 阶段 6 | 已完成：7B-1a～7E 工程实施、机械验证和用户验收均通过 | `confirmed` 范围；分批名称是工程记录，不是新产品模块 |
| 8 | SVG 与摘要渐进增强 | 阶段 7、正式配置准备 | 已完成：工程实现、机械验证和用户验收均通过 | `confirmed` 范围；具体路径、尺寸和示例配色不是产品数据 |
...
阶段 6 以后继续使用同一决策升级机制，并在重要纵向节点增加学习型请求链路 walkthrough。阶段 8 已确认为阶段 7 之后的下一正式阶段；阶段 9～10 的当前依赖顺序仍为 `provisional`。开发 Agent 可以按正式依赖继续细化，但不能借此改变产品范围或把未决产品规则自行确认。
...
| M2／纵向切片1 | email_verification_tokens；rate_limit_keys → send_attempts | M2-A～C 已实施并验证；未新增表，完整闭环已通过用户验收 |
...
**状态：`已完成`（M1、M2-A、M2-B、M2-C 实现与机械验证通过；完整闭环已通过用户验收）**
**决定状态：`confirmed`**
...
- 自动测试已证明本地 test adapter 下的“注册→邮件→只读确认→提交验证→手动登录→受保护空历史→语言切换→退出”闭环；本段记录的是 2026-09-01 交付时的待验收状态，该闭环后续已于 2026-09-05 通过用户验收。
...
### 12.9 本地邮件预览与验证后返回登录（2026-09-02）

**状态：已实现并通过机械验证；用于阶段6本地用户验收。** 用户确认开发环境注册发信后自动打开本地邮件预览，由用户亲自点击邮件内的真实验证链接；系统不得直接绕过邮件打开token URL。二次确认成功后返回登录页并显示成功提示，仍不自动登录。

- development-only 增加 `letter_opener_web` 3.0.0，锁定其 `letter_opener` 1.10.0 与 Launchy 依赖；开发投递方式使用 `:letter_opener` 自动调用本机默认浏览器，同时挂载仅开发环境存在的 `/letter_opener` 备用收件箱。
- test 环境继续使用 `:test` adapter且不存在Letter Opener路由；生产环境未引入该Gem或路由，不发送真实邮件，也没有改变Mailer、Job、token、限流、事务、数据表或Migration。
- 邮箱确认继续使用GET只读检查和POST二次确认。只有首次成功验证才重定向登录页并显示本地化成功提示；无效、过期、已消费、被替代等状态继续进入原结果页。
- development真实冒烟：邮件成功写入临时Letter Opener目录，Launchy生成浏览器打开命令，`/letter_opener`返回200并显示邮件主题；临时邮件随后清理。test配置复核为`:test`且路由未挂载。
- 最终回归：普通测试120个／1261断言（seed27555），System Test 14个／331断言（seed11736），均0失败、错误或跳过；Zeitwerk通过；RuboCop 86文件0违规；Brakeman 79检查0错误、0安全警告；`git diff --check`通过。
- 本批是本地开发和已确认验收流程调整，不是生产邮件方案。自动打开依赖Windows默认浏览器关联；即使自动弹窗失败，用户仍可访问`/letter_opener`完成验收。

## 13. 后续纵向切片候选

**总体决定状态：阶段 7 与阶段 8 范围为 `confirmed`；阶段 9～10 的当前顺序为 `provisional`**

### 13.1 Observation 核心闭环

- Observation 与 User 归属；
- 选择有效轮廓；两层流程、第一层水面／水边／大鸟／小鸟、第二层 16 个具体轮廓与 1 个兜底选项已经确认，完整配置以 `requirements.md` 第 7 节为准；内部稳定 key、17 张本地 SVG 及逐张来源／作者／CC0 许可记录已经落实；
- 首次保存；
- 空／非空历史；
- 详情与重新打开；
- 当前用户作用域授权。
...
阶段 7 实际按“配置收口 → M3 数据结构 → 首次创建与历史 → 详情编辑与并发 → 回归验收”完成工程实施。该顺序只记录开发事实，不构成新的产品模块划分。SVG 与摘要继续留在阶段 8。

#### 13.1.1 阶段 7 实施与验证记录（2026-09-05）

**状态：`已完成`**

- 7B-1a、7B-1b、7B-2、7C、7D、7E 已完成工程实施；M3 新增 `observations`、`part_impressions`、`activity_location_selections` 三张表。
- 真实链路已经覆盖 Observation 创建、非空历史、详情和重新编辑；首次保存要求有效轮廓与至少一个有效 PartImpression，并以同一事务保存父记录、部位和行动位置。
- 所有读取与写入从 `Current.user.observations` 开始；跨用户访问返回未找到。状态变更使用 CSRF，编辑使用 expected revision、父记录行锁和整体事务阻止旧页面静默覆盖新提交。
- 当前并发冲突以 422 重新渲染并保留输入，提示用户返回最新详情后重新编辑；该反馈已包含在阶段 7 用户验收中，具体 HTTP 组织、文案和视觉仍是可调整的实施方案。
- 轮廓页和详情页当前使用静态轮廓素材；动态部位着色 SVG 与文字摘要不进入阶段 7。
- 开发／测试库的 M3 Migration、回滚、重新迁移及 Schema 加载已经通过。开发 Agent 交接报告普通测试 148 个／2764 断言、System Test 15 个／350 断言，均为 0 failures、0 errors、0 skips；RuboCop、Brakeman、Zeitwerk、JavaScript 语法和差异检查均通过。
- 用户已验收 Observation 创建、历史、详情、编辑和当前并发冲突反馈，阶段 7 完成。17 张轮廓选择素材的逐张裁切／尺寸优化与真实设备检查仍可继续细化，但不再阻塞阶段 8。

### 13.2 SVG 与摘要渐进增强

**状态：阶段 8 `已完成`；范围：`confirmed`；工程实现、机械验证和用户验收均通过**

- 复用上一切片已有的部位、颜色、特征、确定程度、自由文字、行动位置与行为数据；最低保存条件已在 Observation 首次保存时实现，本阶段不补建这一前置条件；
- 从结构化观察数据生成服务器端 inline SVG 与简体中文／日文特征摘要；编辑页提供渐进预览，详情页和历史缩略图从已保存数据重新生成；
- 17 个轮廓分别维护头部、胸腹、翼、尾四个矢量分区，不采用一套通用粗略遮罩；
- 抽象图采用简洁扁平的大色块；每个身体部位使用一个完整主色区，最多增加一个连续补充色区，不能在主色和补充色之外叠加第三层视觉模块；
- 翼部最多使用一个内部补充色区，翼镜、翼斑、翼带等只决定该色区的形状与位置；胸腹补充色沿腹部形成连续大面积解剖分区，并按每个轮廓的体形自然抬高至前胸；
- 内部补充色区不使用黑色或深色描边；鸟体外轮廓以及头部、胸腹、翼、尾之间继续保留深色边线，以维持缩略图辨识度；
- SVG 始终使用 `outline_key` 对应的基础轮廓；冠羽、叉尾和长尾只进入摘要，不生成、叠加或替换局部外形。自由文字和确定程度同样只进入摘要，不参与 SVG 映射；
- 阶段 8A 的鸭科 `anatidae`、鹭科 `ardeidae`、圆身小鸟 `compact_passerine` 三个代表性试稿已经获得用户对视觉语法的确认；用户进一步确认并接受鸭科水面鸭、鹭科站立鹭的正式来源与实际视觉方向，继续使用来源可追溯、许可明确的 PhyloPic 几何，不改变稳定 key、表结构或 MVP 范围；站立鹭只是代表性视觉参考，MVP 不新增其他鹭科姿态变体；
- 全部正式抽象鸟图使用已登记、可追溯的 PhyloPic SVG 作为真实外边界，内部身体分区和色块由服务器端 inline SVG 组合；`compact_passerine` 已改用其已登记的 `Cyanistes caeruleus` 外边界，翼部只保留一个完整主色区域和一个连续、无深色描边的补充色区，翼带、翼斑、翼镜只改变这一个内部区域；
- 阶段 8 不新增 Migration、数据库字段或表，不持久化 SVG、摘要或缩略图；鸟种识别、密码辅助与部署不进入本阶段。

**暂定工程实现**：选择项即时触发预览，自由文字约 350ms 防抖；预览请求串行执行，只保留最新待发送状态。具体文件名、Ruby 类名和版本控制配置组织由开发 Agent 在既有边界内决定。

**当前实施证据与已确认边界**：鸭科与鹭科换源、圆身小鸟登记来源外边界和三个代表性试稿已经完成并获得前述视觉确认。开发提交 `46c7900` 完成 17 个轮廓映射，`dce876e` 接入本地化摘要与详情／历史重新生成，`32ceee5` 接入 Turbo／Stimulus 渐进预览；当前实现已移除冠羽、叉尾和长尾局部 SVG 变体。开发任务报告 Rails 174 个测试／3827 个断言、System Test 19 个／380 个断言，均为 0 失败、0 错误、0 跳过；RuboCop 113 个文件／0 违规，Brakeman 0 警告，Zeitwerk、JavaScript 语法和差异检查通过，`db/schema.rb` 无变化。阶段 8 已完成工程实现、机械验证和用户验收；其余 14 个轮廓已有机械映射和内部检查证据，但尚未逐一获得用户视觉确认，这一已知覆盖缺口不阻塞阶段完成。纵纹、横斑、斑点、杂斑、金属光泽、淡色羽缘、冠羽、叉尾和长尾只进入文字摘要；色块型特征与补充色可以通过每部位最多一个连续固定辅助区域表达，不得恢复密集纹理、结构型局部轮廓变体或第三层模块。
...
| 2026-09-02 | 阶段6／本地验收工具 | 本地邮件预览已接入 | 开发环境自动打开邮件并保留`/letter_opener`备用收件箱；确认成功回登录且不自动登录；120个普通测试／1261断言、14个System Test／331断言全通过；详见12.9 |
| 2026-09-05 | 阶段6 → 阶段7 | 阶段6已完成，阶段7已进入 | 用户确认阶段6验收门槛通过，可以进入Observation核心闭环；本轮只形成执行计划，尚未实施M3或修改业务代码 |
| 2026-09-05 | 阶段7／配置收口 | 第一层轮廓逻辑已确认 | 保持两层选择，第一层为水面／水边／大鸟／小鸟，第二层按目分段并以图片为主；具体目科映射、轮廓 key、复用名称与素材仍待核对，未修改代码或 Migration |
| 2026-09-05 | 阶段7A-1／正式配置 | 产品配置清单已确认 | 确认16个具体轮廓加1个兜底、12个颜色、身体部位与特征及11个行动位置；日本分类以《日本鳥類目録 改訂第8版》为主并用AviList交叉检查；81个ZooPicker鸟种仅作地点覆盖样本；素材、内部key和页面排版仍待实施，未修改代码或Migration |
| 2026-09-05 | 阶段7／7B-1a～7E | 实现与机械验证通过，待用户验收 | M3三表、真实创建／历史／详情／编辑、授权、CSRF、expected revision、父锁与聚合事务已实现；普通测试148个／2764断言、System Test 15个／350断言全通过，质量检查通过；动态SVG／摘要与鸟种识别未提前实施 |
| 2026-09-06 | 阶段7 | 已完成 | 用户验收 Observation 创建、历史、详情、编辑和当前并发冲突反馈；阶段 7 不再处于待验收状态 |
| 2026-09-06 | 阶段8／范围收口 | 下一正式开发阶段 | 确认17套四分区映射、结构特征局部变形、固定补充色区域及三小样先行；自动预览节奏仍为暂定工程实现，不新增数据库结构 |
| 2026-09-06 | 阶段8A／代表性试稿 | 来源方向已确认，换源效果待验收 | 用户确认视觉语法，并确认鸭科采用水面鸭、鹭科采用站立鹭的明确许可 PhyloPic 来源；开发任务报告换源与专项测试通过，但改动未提交、未推送，阶段 8A 尚未完成 |
| 2026-09-06 | 阶段8A／鸭科与鹭科 | 实际视觉方向已验收 | 用户接受水面鸭和站立鹭的当前视觉方向，并确认全部正式抽象鸟图使用已登记 PhyloPic 外边界；圆身小鸟改造与测试仍未完成，阶段 8A 保持进行中 |
| 2026-09-07 | 阶段8A／圆身小鸟 | 工程实现完成 | `4955c3e` 接入已登记 `Cyanistes caeruleus` 外边界和单一翼内连续补充色区；24 个相关测试／587 断言、17 个 System Test／365 断言全通过，320px 日文与 390px 中文截图已检查；细粒度特征映射仍未确认 |
| 2026-09-07 | 阶段8／特征映射 | 产品边界已确认 | 细粒度花纹只进文字摘要；冠羽、叉尾、长尾只局部改变当前基础轮廓并可恢复，不改变轮廓 key、分类或来源；其余 14 个轮廓可继续实施 |
| 2026-09-07 | 阶段8／结构特征修正 | 新决定取代旧边界 | 用户确认冠羽、叉尾、长尾只进入双语摘要，不再改变 SVG 外轮廓；先前局部变体方向已否决 |
| 2026-09-07 | 阶段8／完整实现 | 工程与机械验证完成，待用户验收 | `46c7900` 完成 17 个轮廓映射，`dce876e` 接入本地化摘要与详情／历史重新生成，`32ceee5` 接入渐进预览；Rails 174／3827、System 19／380 全通过，质量检查通过，Schema 未变；其余 14 个轮廓待用户视觉确认 |
| 2026-09-07 | 阶段8／用户验收 | 已完成 | 用户确认继续，阶段 8 整体验收通过；其余 14 个轮廓未逐一视觉确认作为已知覆盖缺口保留，不阻塞阶段完成；下一开发阶段尚未讨论或确认 |

## 18. 当前阻塞与下一步

### 当前阻塞

阶段 8 已完成工程实现、机械验证和用户验收，当前没有阻塞阶段完成的未决产品问题。其余 14 个轮廓尚未逐一获得用户视觉确认，作为已知视觉覆盖缺口保留。

### 当前下一步

下一开发阶段尚未讨论或确认，不从既有暂定顺序中自动选择。冠羽、叉尾和长尾只进入摘要，不得恢复局部 SVG 变体；鸟种识别、密码辅助与部署仍留在后续阶段。只有遇到新的产品、风险或视觉验收门槛时才升级询问用户。
