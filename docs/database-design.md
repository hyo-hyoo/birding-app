...
> 建立日期：2026-08-17  
> 当前状态：阶段 2～3 设计基线及阶段 5 迁移映射已完成核查；M1 User／Session 两表及认证基础已实施并验证，其余批次尚未实施
...
实际落地范围：M1 已有 [CreateUsers](../db/migrate/20260831000001_create_users.rb)、[CreateSessions](../db/migrate/20260831000002_create_sessions.rb) 两份迁移；当前 [schema.rb](../db/schema.rb) 只包含 `users`、`sessions` 两张业务表。开发／测试库执行与结构往返证据见 [backend-development-plan.md](backend-development-plan.md) 12.5 节，其余八表仍是设计，不是已建表结果。
...
状态 CHECK 的精确布尔表达式如下（已用本机 MySQL 只读 SELECT 验证有效与非法／NULL 状态，不等于已经建表验证）：
...
以下是数据库设计必须支持的既有业务规则；设计见第10、12～15节。M1 只实现账户与 Session 基础，不代表下列全部流程已实现；注册、邮箱验证、正式页面及后续业务仍待对应批次完成。
...
这些是完整 MVP 的设计约束。M1 已对 `users`、`sessions` 验证真实 SQL 非法写入拒绝、唯一竞争及相关认证并发；不能据此认为其他表的约束已通过。M2～M5 仍须逐批用绕过 Rails Validation 的 SQL 反例测试验证数据库保护。
...
实施边界：M1 已验证当前 Session 退出删除及到期立即不可认证；过期 Session 次日维护命令尚未实施，须在阶段 6 结束前落实。令牌和限流清理随其后续批次实施，不能把即时失效判断当作已完成物理清理。
...
工程协议（后续 M2 必须以故障注入和真实线程测试验证；本阶段只完成设计）：
...
以上为完整 MVP 的设计和测试要求。M1 仅有认证基础及相应测试证据；正式页面认证、邮件／日志隐私全链路、维护清理和生产安全验证仍未完成，不能外推其覆盖范围。
...
阶段6的M1 User／Session及生成器最小认证基础已实施，迁移文件对应见第9节；密码重置仍延后。接着M2完成注册验证和正式页面串联后，才验收端到端“注册→验证→手动登录→空历史→退出”。不能用测试专用入口或跳过验证的临时登录替代正式切片。
...
- M2～M5仍按各批次验证结构和业务约束。M2必须执行14.6全部故障注入／并发检查，以及日志过滤与维护清理；不能只用test adapter的成功入队记录或M1测试作为证明。密码辅助实施时仍需回归14.5的登录竞态。
...
M1已落地，按后端计划继续阶段6的M2邮箱验证令牌、限流结构及相关流程，不一次性创建其余表。每批先审查最小改动及测试，再执行相应迁移；设计文档不能代替数据库、并发和请求测试。阶段状态与验证计数只在后端计划维护。
