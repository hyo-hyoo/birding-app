...
- Session 退出、过期或撤销最终采用物理删除还是失效标记，由后续物理数据库设计决定；该实现选择不得改变“数据库 Session 是登录事实来源”的边界。
...
- 用户主动退出时只删除或失效当前浏览器对应的当前 Session 记录，不批量结束其他浏览器或设备上的会话。
- 在基础认证之上实现注册和邮箱验证。
...
- 每次成功生成并切换到新的验证令牌后，先前令牌立即失效，同一时间只有最新验证链接有效；
- 验证链接成功使用后立即失效，不能重复使用；
- 如果新邮件发送流程失败，不切换掉当前仍有效的旧令牌；
...
**暂定实施点**：在已经确认使用 `deliver_later` 的前提下，“新邮件成功发出”的精确事务边界仍需实现验证。当前建议以“新令牌成功保存且邮件任务成功入队”为切换点；异步任务入队后的极端投递失败如何处理仍属于实施细节，不得在验证前写成已完成行为。
...
- 注册已创建未验证 User、但邮件任务未能成功入队时，保留该 User、不创建 Session，并进入可由统一重发流程恢复的结果状态。成功重发可以建立新令牌；失败重发不能使原有有效令牌失效。
...
- Session 最终采用物理删除还是失效标记，以及过期记录清理策略，留给物理数据库设计，不改变上述行为。
...
- 同一业务范围内，旧页面不能静默覆盖较新提交。Observation 编辑与 BirdIdentification 是两个独立并发范围；具体采用乐观锁、版本号或其他机制仍由阶段 2 设计确认。
...
- **数据库持久化**：User、Session、邮箱验证和密码重置业务状态与令牌、限流证据、Observation、PartImpression、ActivityLocationSelection、候选鸟名和最终鸟名。具体对象和字段映射仍由数据库设计确认。
...
- Windows `mysql2` 与 MySQL 8.4.10 的 TLS 问题可以与后端和数据库设计并行处理，但必须在执行业务 Migration 和数据库业务测试前解决并完成验证。
...
- **当前实施阻塞**：完整数据库相关测试目前在 Windows `mysql2` 与 MySQL 8.4.10 的 TLS 协商阶段中止。此前基础连接和健康检查成功仍然有效，但不能代替当前业务测试环境验证。解决方式尚未确认：仅在本地 `development`／`test` 关闭证书验证，或让 `mysql2` 改用 Oracle MySQL 客户端库并正确配置 TLS；在用户确认并实际复测前不得自行选定方案。
