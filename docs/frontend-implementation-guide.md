...
> 当前适用阶段：静态前端 14／14、阶段 6 账户闭环与阶段 7 Observation 核心闭环均已验收；阶段 8A 视觉语法及鸭科／鹭科实际视觉方向已确认，圆身小鸟改造与测试待完成<br>
> 最近核对日期：2026-09-06<br>
...
- `compact_passerine` 的翼部只保留一个完整主色区域和最多一个连续补充色／特征区域；翼带、翼斑、翼镜只决定该唯一内部区域的形状和位置，不再叠加“白色色块＋两条翼带”或其他独立小模块；
...
**当前开发证据**：开发任务报告两个本地 SVG 源文件和 observation options 来源元数据已经替换，渲染器已直接读取可信源几何且内部补充色无深色描边；相关普通测试 35 个／1817 断言、阶段 8A 专项 System Test 2 个／15 断言均为 0 失败、0 错误、0 跳过。鸭科与鹭科当前视觉方向已经用户接受；圆身小鸟尚未完成正式来源外边界、单一翼内区域改造和测试，因此不能把阶段 8A 标记完成。
...
| 抽象鸟图 | 高保真基线展示概念性鸟图 | 开发工作区已为鸭科、鹭科更换水面／站立 PhyloPic 源几何并获用户视觉接受；圆身小鸟仍待正式来源外边界、单一翼内区域改造与测试 | 三个试稿确认大色块、每部位最多一个连续补充色区等视觉语法；全部正式外边界来自登记的 PhyloPic SVG，内部由 inline SVG 组合，旧实现和临时试稿路径都不是正式资产 |
...
- 轮廓选择当前已有 17 张本地 PhyloPic CC0 SVG 及逐张许可记录；阶段 8A 三个代表性试稿的视觉语法已经确认，鸭科水面鸭与鹭科站立鹭的新来源和实际视觉方向也已确认。圆身小鸟仍待改用其已登记外边界、收敛为单一翼内区域并完成测试；动态部位着色鸟图、摘要和历史缩略图整体仍未完成；
...
- 轮廓选择素材与阶段 8 生成映射必须区分：17 张轮廓选择 SVG 已落地并记录 CC0 来源；17 套四分区映射、结构特征局部变化和单一连续补充色区域已经确认。鸭科、鹭科的新来源及实际视觉方向已经验收；圆身小鸟改造、后续矢量绘制和相关测试仍未完成；
