...
> 当前适用阶段：静态前端 14／14、阶段 6 账户闭环与阶段 7 Observation 核心闭环均已验收；阶段 8 工程实现与机械验证已完成，等待用户最终验收<br>
...
- Observation 的两层轮廓选择、新建、非空历史、详情和重新编辑已经接入真实 Route、Controller、Model、三张 M3 表和服务器校验；动态 SVG、本地化摘要、详情／历史重新生成和渐进预览也已接入并通过机械验证，仍待阶段 8 用户验收；鸟种识别、密码重置和修改密码尚未接入；
...
- 正式提交只保存轮廓稳定 key，不把页面分段标题或科名提交为鸟类鉴定结果。分类元数据与轮廓素材引用分离；16 个具体轮廓、1 个兜底选项及颜色、特征、行动位置的正式清单以 `requirements.md` 第 7 节为准，不得继续采用静态预览旧示例；内部 key、具体 SVG 和逐张许可记录已经落实，当前视觉效果仍须服从阶段 8 用户验收；
...
**当前开发证据**：鸭科与鹭科换源、圆身小鸟登记来源外边界和三个代表性试稿已经完成并获得前述视觉确认。开发提交 `46c7900` 完成 17 个轮廓映射，`dce876e` 接入本地化摘要与详情／历史重新生成，`32ceee5` 接入 Turbo／Stimulus 渐进预览；当前代码已移除冠羽、叉尾和长尾局部 SVG 变体。开发任务报告 Rails 174 个测试／3827 个断言、System Test 19 个／380 个断言，均为 0 失败、0 错误、0 跳过；RuboCop 113 个文件／0 违规，Brakeman 0 警告，Zeitwerk、JavaScript 语法和差异检查通过，`db/schema.rb` 无变化。以上支持阶段 8 工程实现与机械验证完成，但不替代用户最终验收；其余 14 个轮廓尚未逐一获得用户视觉确认。
...
| 抽象鸟图 | 高保真基线展示概念性鸟图 | 17 个轮廓均已接入登记来源外边界和四分区映射，阶段 8 工程实现完成并待用户验收 | 全部正式外边界来自登记的 PhyloPic SVG，内部由 inline SVG 组合；摘要专用特征不改变 SVG，色块型特征与补充色使用固定辅助区域。其余 14 个轮廓仍待用户逐一确认视觉效果 |
| 历史缩略图 | 原型展示不同鸟类轮廓 | 已从保存的结构化数据重新生成阶段 8 SVG | 生成后的缩略图不持久化；当前效果仍包含在阶段 8 待验收范围内 |
...
- 轮廓选择已有 17 张本地 PhyloPic CC0 SVG 及逐张许可记录；阶段 8 的 17 个动态部位着色映射、本地化摘要、详情／历史重新生成和渐进预览已经完成工程实现与机械验证。其余 14 个轮廓尚未逐一获得用户视觉确认，阶段 8 仍待最终验收；
...
- 轮廓选择素材与阶段 8 生成映射必须区分：17 张轮廓选择 SVG 已落地并记录 CC0 来源，17 套四分区映射也已实现。冠羽、叉尾和长尾只进入摘要，先前局部 SVG 变体已经否决并移除；其余 14 个轮廓仍待用户逐一确认视觉效果；
