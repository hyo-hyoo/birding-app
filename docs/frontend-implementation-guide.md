...
> 当前适用阶段：静态前端 14／14 已验收；后端 M1 与 M2-A 基础已验证，注册／邮件和正式页面接入仍未完成<br>
...
- 后端已有 M1 认证及 M2-A 令牌／数据库限流基础，但尚未接入正式业务路由或 ApplicationController；这些页面仍不读写真实账户或观察数据，不能把五张业务表已建立理解为页面已接通；
- 刷新页面会重置浏览器内的交互状态；
...
M1 认证和 M2-A 邮箱验证请求测试使用独立测试 Controller 与路由；测试响应格式、脱敏展示不属于正式接口或 UI 决定。测试令牌夹具不代表真实签发／重发已实现。后续接入应复用现有前端布局和组件，不能把测试入口接给用户；工程拆分和验证证据见 [backend-development-plan.md](backend-development-plan.md) 12.5～12.6 节，正式页面与 locale 串联暂留 M2-C。
...
- 当前预览页面没有真实业务表单提交、认证接入、业务持久化或服务器校验；后端基础测试通过不代表这些页面已经接通，也没有完成业务 Turbo 响应链路；
