...
> 当前适用阶段：静态前端 14／14 已验收，进入第一个业务纵向切片的后端接入准备<br>
...
- 用户可直接修正的问题使用字段错误；登录失败等整体错误使用不区分“邮箱不存在”和“密码错误”的页面级通用提示；
- 验证邮件已发送、验证成功、链接过期、链接无效和已经验证等流程结果由 Controller 根据真实服务器状态选择结果页面或 `_result_card`；
- 邮件服务内部故障只向页面提供通用提示，具体异常写入服务器日志；限流反馈同样不得泄露邮箱是否存在；
- 退出登录提交到正式退出端点，只结束当前浏览器对应的当前 Session；页面不增加“记住我”选项；
- 真实后端页面优先复用现有布局、账户 Partial、`_form_errors` 和 `_result_card`，不为后端接入另建视觉体系。

**暂定实施边界**：静态预览路由是否长期保留，由开发 Agent 在不影响正式路由的前提下判断。验证邮件使用 `deliver_later` 时，新令牌切换与邮件任务入队的精确事务边界以 `technical-decisions.md` 的暂定项和后续实施验证为准。
...
- 完整数据库相关测试当前会在 Windows `mysql2` 与 MySQL 8.4.10 的 TLS 协商阶段中止；该问题不影响已完成的静态前端验收，但会阻塞真实数据库业务接入，解决方案仍待确认。
