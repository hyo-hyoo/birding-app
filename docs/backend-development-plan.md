...
> 建立日期：2026-08-17  
> 当前阶段：阶段 7 已完成；阶段 8A 的视觉语法及鸭科／鹭科实际视觉方向已确认，圆身小鸟改造与测试待完成
...
| 8 | SVG 与摘要渐进增强 | 阶段 7、正式配置准备 | 进行中：阶段 8A 视觉语法及鸭科／鹭科实际视觉方向已确认，圆身小鸟改造与测试待完成 | `confirmed` 范围；具体路径、尺寸和示例配色不是产品数据 |
...
**状态：`进行中`；范围：`confirmed`；阶段 8A 视觉语法及鸭科／鹭科实际视觉方向已确认，圆身小鸟改造与测试待完成**
...
- 全部正式抽象鸟图使用已登记、可追溯的 PhyloPic SVG 作为真实外边界，内部身体分区和色块由服务器端 inline SVG 组合；`compact_passerine` 下一步改用其已登记来源外边界，翼部只保留一个完整主色区域和最多一个连续补充色／特征区域，翼带、翼斑、翼镜只改变这一个内部区域；
...
**当前实施证据与待确认项**：开发任务报告两个本地 SVG 源文件及来源元数据已经替换，渲染器直接读取版本库中的可信 PhyloPic SVG 几何并把大色块裁切在轮廓内部，内部补充色无深色描边；普通测试 35 个／1817 断言、阶段 8A 专项 System Test 2 个／15 断言均为 0 失败、0 错误、0 跳过。鸭科与鹭科当前视觉方向已经用户接受；圆身小鸟的正式来源外边界、单一翼内区域改造和测试仍未完成，因此阶段 8A 不标记完成。纵斑、横斑、斑点、杂斑、金属光泽和淡色羽缘等细粒度特征，哪些只进入摘要、哪些可以在不增加视觉层级的前提下转化为一个大面积色区，也仍未确认。`requirements.md` 6.7 当前要求抽象图表达已记录的预设花纹或特征；若拟让某些标签只进入摘要，必须作为产品行为变化升级给用户，开发 Agent 不能自行省略。其余轮廓的具体路径需要逐一实现和检查。旧密集纹理、固定补充色加多特征叠层、旧示例配色和概念鸟体均是 observed-only，不是正式生成规则。
...
鸭科与鹭科的外轮廓来源和实际视觉方向已经由用户确认，不再构成决定阻塞。当前阶段 8A 仍需完成圆身小鸟的正式来源外边界、单一翼内区域和相关测试，在完成前不能标记阶段 8A 完成。用户只选择特征但没有补充色时如何在图中表现，以及细粒度花纹如何在不增加视觉层级的前提下图形化，仍是开放问题，也不得恢复旧密集纹理。

### 当前下一步

下一步由开发 Agent 把圆身小鸟改为其已登记的 PhyloPic 外边界，并将翼部收敛为一个完整主色区域和最多一个连续补充色／特征区域，完成相关测试后再判断阶段 8A 是否完成。随后按同一套大色块、单一连续补充色区、胸腹连续分区、内部色块无描边和结构特征改轮廓规则扩展其他轮廓，并检查缩略图辨识度。中日文摘要与不写数据库的渐进预览继续按既定范围实施；鸟种识别、密码辅助与部署仍留在后续阶段。只有遇到新的产品、风险或视觉验收门槛时才升级询问用户。
