...
## Project Compatibility (Local Addition)

In this repository, `AGENTS.md`, the formal project documents, and the UI Design / Frontend Prototype Agent instructions take precedence over this skill. Apply the design principles below to the browser-based Web App while using Rails Views, HTML, CSS, JavaScript, Turbo, and Stimulus as the default implementation direction.

Web-capable tools such as Tailwind CSS, React, Lucide React, Recharts, React Native through a web target, or Flutter Web may be considered when they clearly fit the approved scope, but they are examples rather than required technologies. Explain and justify any new framework or dependency before introducing it. SwiftUI may be used as design inspiration, but it is not a browser Web App implementation technology and must not be introduced as one. Native iOS and Android applications remain outside the current MVP.
