...
> 当前适用阶段：静态前端 14／14、阶段 6 账户闭环与阶段 7 Observation 核心闭环均已验收；阶段 8A 三个代表性轮廓的工程实现已完成，细粒度特征映射待确认<br>
...
- 17 个轮廓分别拥有头部、胸腹、翼、尾四个矢量分区，不使用一套通用粗略遮罩；冠羽、叉尾、长尾等结构特征可以切换相应局部矢量表现；
...
**仍待确认的映射边界**：用户只选择特征而没有补充色时如何在图中表现；纵斑、横斑、斑点、杂斑、金属光泽和淡色羽缘等细粒度特征，哪些只进入摘要、哪些可在不增加视觉层级的前提下转化为一个大面积色区。`requirements.md` 6.7 仍要求抽象图表达已记录的预设花纹或特征；如果要让某些标签只进入摘要，必须先由用户确认该产品行为变化。未确认前不得自行省略，也不得沿用旧密集斜线、重复纹理或多个独立小模块。

**当前开发证据**：鸭科与鹭科换源实现及前一轮专项测试已完成，两者当前视觉方向已经用户接受。开发提交 `4955c3e` 又把圆身小鸟接到已登记的 `Cyanistes caeruleus` 源几何，移除两条翼带和多个独立小模块；相关 Model／Integration 测试 24 个／587 断言、完整 System Test 17 个／365 断言均为 0 失败、0 错误、0 跳过。开发任务重新生成并检查了 320px 日文与 390px 中文截图，三个小样均使用已登记来源外边界，内部补充色为连续大色块。以上证据支持阶段 8A 工程实现完成，不表示尚未确认的特征映射边界已经解决。
...
| 抽象鸟图 | 高保真基线展示概念性鸟图 | 鸭科、鹭科已更换水面／站立 PhyloPic 源几何并获用户视觉接受；圆身小鸟也已接入登记来源外边界和单一翼内区域，阶段 8A 工程实现完成 | 三个试稿确认大色块、每部位最多一个连续补充色区等视觉语法；全部正式外边界来自登记的 PhyloPic SVG，内部由 inline SVG 组合。细粒度特征映射仍未确认，旧实现和临时试稿路径都不是正式资产 |
