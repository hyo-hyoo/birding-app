...
> 建立日期：2026-08-17  
> 当前状态：阶段 2～3 设计基线及阶段 5 迁移映射已完成核查；尚未创建业务 Model、Migration 或业务表
...
以下是数据库设计必须支持的既有业务规则；本轮设计见第10、12～15节，实际业务实现尚未开始：
...
这些是设计约束，不表示已存在 SQL 表；M1～M5 实施时须用绕过 Rails Validation 的 SQL 反例测试验证数据库真正拒绝非法数据。
...
以上为设计和测试要求，不声称已实现认证、日志过滤、清理或生产安全。
...
阶段6第一个小任务：先接入M1的User／Session及生成器最小基础，审查生成器带出的密码重置文件并延后，配套模型／数据库测试；接着M2完成注册验证后才验收端到端“注册→验证→手动登录→空历史→退出”。不能用跳过验证的临时登录交付替代正式切片。
...
本轮设计交付接受核查后，按后端计划进入阶段6的M1小任务，不一次性创建全部表。每批先审查最小改动及测试，再执行相应迁移；设计文档不能代替数据库、并发和请求测试。阶段状态与验证计数只在后端计划维护。
