...
> 建立日期：2026-08-17  
> 当前阶段：阶段 7 已完成；阶段 8A 的视觉语法已确认，但代表性试稿的外轮廓来源冲突待用户确认，阶段 8A 尚未完成
...
| 8 | SVG 与摘要渐进增强 | 阶段 7、正式配置准备 | 进行中：阶段 8A 视觉语法已确认，外轮廓来源冲突待用户确认 | `confirmed` 范围；具体试稿路径与来源合规未确认 |
...
**状态：`进行中`；范围：`confirmed`；阶段 8A 视觉语法已确认，外轮廓来源冲突待确认**
...
- 阶段 8A 的鸭科 `anatidae`、鹭科 `ardeidae`、圆身小鸟 `compact_passerine` 三个代表性试稿已经获得用户对视觉语法的确认，但这不确认其具体外轮廓或来源合规；当前登记的鸭科与鹭科 PhyloPic 素材是飞行姿态，独立试稿却是水面鸭和站立鹭，处理方式需要用户确认后才能继续；
...
**待确认与待实施**：鸭科与鹭科试稿姿态不符合当前登记 PhyloPic 源轮廓，外轮廓来源处理需要用户确认；纵斑、横斑、斑点、杂斑、金属光泽和淡色羽缘等细粒度特征，哪些只进入摘要、哪些可以在不增加视觉层级的前提下转化为一个大面积色区，也仍未确认。`requirements.md` 6.7 当前要求抽象图表达已记录的预设花纹或特征；若拟让某些标签只进入摘要，必须作为产品行为变化升级给用户，开发 Agent 不能自行省略。剩余 14 个轮廓的具体路径需要逐一实现和检查。当前 Rails 渲染器和静态前端 `_impression_bird` 中较早的密集纹理、固定补充色加多特征叠层、旧示例配色和概念鸟体均是 observed-only，不是正式生成规则。三个试稿的具体路径、尺寸和示例配色同样不是产品数据。
...
| 2026-09-06 | 阶段8A／代表性试稿 | 视觉语法已确认，外轮廓待确认 | 用户确认简洁扁平大色块、每部位最多一个连续补充色区、内部色块无深色描边；鸭科、鹭科试稿姿态与已登记 PhyloPic 源轮廓不一致，阶段 8A 尚未完成 |

## 18. 当前阻塞与下一步

### 当前阻塞

阶段 8A 当前存在一个需要用户确认的约束冲突：独立试稿中的鸭科水面姿态、鹭科站立姿态与仓库登记的飞行姿态 PhyloPic 源轮廓不一致。视觉语法已经确认，但在外轮廓来源处理明确前，不能把试稿视为来源合规资产或把阶段 8A 标记完成。细粒度花纹如何在不增加视觉层级的前提下图形化仍是开放问题，也不得恢复旧密集纹理。

### 当前下一步

下一步先由用户确认鸭科和鹭科试稿的外轮廓来源处理方式。确认后，开发 Agent 再让 Rails 渲染器完整采用已确认的大色块、单一连续补充色区、胸腹连续分区、内部色块无描边和结构特征改轮廓规则，并扩展其余 14 个轮廓、检查缩略图辨识度。中日文摘要与不写数据库的渐进预览继续按既定范围实施；鸟种识别、密码辅助与部署仍留在后续阶段。
