...
> 当前适用阶段：静态前端 14／14、阶段 6 账户闭环、阶段 7 Observation 核心闭环及阶段 8 SVG／摘要增强均已验收<br>
> 最近核对日期：2026-09-07<br>
...
当前 Rails 前端同时包含已验收的静态预览、阶段 6 正式账户页面和阶段 7 正式 Observation 页面：
...
- 静态预览页面的状态由查询参数、ERB 示例数据和浏览器内存共同模拟；正式业务页面由服务器与数据库状态决定；
- 注册、统一重发、两步邮箱验证、登录、退出、受保护空观察历史、设置与语言选择已接入正式 Route、Controller、View、Session 和五张账户业务表；
- Observation 的两层轮廓选择、新建、非空历史、详情和重新编辑已经接入真实 Route、Controller、Model、三张 M3 表和服务器校验；动态 SVG、本地化摘要、详情／历史重新生成和渐进预览也已接入，并通过机械验证与阶段 8 用户验收；鸟种识别、密码重置和修改密码尚未接入；
- 刷新静态预览会重置浏览器内的模拟状态；正式 Observation 刷新后从数据库重新读取已保存数据；
- 静态预览中的账户表单仍只展示结构和流程；正式账户表单已经使用真实 action、服务器校验和 CSRF 保护；
- 预览路由只在 Rails 本地环境中开放，正式业务状态不由预览查询参数决定。

这一状态是实现条件，不代表未来正式业务页面仍应使用静态数据。

M1 认证测试仍保留独立测试 Controller 与路由；其响应格式和脱敏展示不属于正式接口或 UI 决定。M2-B／C 已在不暴露测试入口的前提下复用现有布局和组件，完成真实签发／重发、正式页面与 locale 串联。工程拆分和验证证据见 [backend-development-plan.md](backend-development-plan.md) 12.5～12.8 节。当前路径命名、内部 Cookie／Session 键名、Post／Redirect／Get 方式和服务器反馈微文案属于工程实现，可在不改变正式规则的前提下调整。

### 3.3 账户页接入真实后端的边界

阶段 6 的正式账户页面已使用 Rails 服务端渲染，当前数据流为：
...
- 邮箱验证链接首次 GET 只由服务器检查并显示带脱敏邮箱的确认页；用户确认 POST 时服务器再次校验并完成验证。验证成功返回登录页显示成功提示但不创建 Session；链接过期、无效、已使用、已被替换和已经验证等状态由 Controller 选择结果页面或 `_result_card`；
...
**已确认邮件反馈边界**：新令牌保存且邮件任务实际入队成功才切换链接；入队不等于送达，结果页面不能承诺邮件必达。入队失败保留旧链接，后续投递失败通过重试或重发恢复而不恢复旧链接；失效令牌清理后的链接统一显示无效。正式规则见 `technical-decisions.md` 6.1、6.8 节；事务和 Job 协作见 `database-design.md` 14.3 节，并已在 M2-B／C 完成当前本地实现验证。生产队列和真实邮件投递仍须在部署阶段重新验证。

**暂定实施边界**：静态预览路由是否长期保留，由开发 Agent 在不影响正式路由的前提下判断。

### 3.4 观察记录与鸟种识别接入真实后端的边界

- 轮廓选择保持两层：第一层为水面、水边、大鸟、小鸟，第二层按“目”使用标题分段并以轮廓图片为主要选择依据；不得增加需要用户点击的第三层“目”选择。页面必须说明选择轮廓不等于确认科或物种；具体中日文文案可以微调但不能改变含义；
- 正式提交只保存轮廓稳定 key，不把页面分段标题或科名提交为鸟类鉴定结果。分类元数据与轮廓素材引用分离；16 个具体轮廓、1 个兜底选项及颜色、特征、行动位置的正式清单以 `requirements.md` 第 7 节为准，不得继续采用静态预览旧示例；内部 key、具体 SVG 和逐张许可记录已经落实，并已纳入阶段 8 整体验收；
- 颜色选项必须同时显示文字和明确选中状态；身体部位使用头部／胸腹／翼／尾的正式显示名称，特征只能出现在其允许的通用或部位专属范围内；“空中”只是行动位置，不能触发飞行轮廓或新的识别流程；
...
**当前阶段 7 实现事实**：正式 Observation 页面已经按上述边界接入真实持久化，并通过用户验收。编辑提交携带 expected revision，服务器在父记录行锁和整体事务中比较版本；当前冲突响应为 422 重新渲染、保留输入，并提示返回最新详情后重新编辑。当前反馈已包含在验收范围内，但具体 HTTP 组织、文案和交互仍可调整，不得取消防止旧页面覆盖新数据的规则。

### 3.5 阶段 8 SVG、摘要与渐进预览边界

- 服务器只读取结构化表单或已保存 Observation 数据，生成 inline SVG 与简体中文／日文摘要；预览不保存 Observation，详情与历史缩略图也不保存生成结果；
- 17 个轮廓分别拥有头部、胸腹、翼、尾四个矢量分区，不使用一套通用粗略遮罩；SVG 始终使用 `outline_key` 对应的基础来源几何，冠羽、叉尾和长尾只进入本地化摘要，不生成、叠加或替换局部矢量表现；
- 使用简洁扁平的大色块；每个身体部位只有一个完整主色区和最多一个连续补充色区，不能再叠加第三层视觉模块；
- 翼镜、翼斑、翼带等只能改变唯一翼内补充色区的形状和位置。胸腹补充色使用沿腹部连续、并按轮廓体形向前胸自然抬高的大面积分区，不使用漂浮的小贴块；
- 内部补充色区不使用黑色或深色描边；鸟体外轮廓及头部、胸腹、翼、尾之间继续使用深色边线。部位自由文字与确定程度只进入本地化摘要，不参与图形映射；
- 映射使用版本控制配置。具体文件名、Ruby 类名和对象拆分是可调整的工程实现，不能反向改变正式 UI 或产品规则；
- `anatidae`、`ardeidae`、`compact_passerine` 三个代表性试稿已经验证并确认上述视觉语法。用户也已接受 `anatidae` 水面鸭和 `ardeidae` 站立鹭换源后的实际视觉方向；“站立鹭”只作为鹭科代表性视觉参考，不表示所有鹭科都采用同一伸颈姿态，也不构成科或物种确认。MVP 接受单一站立鹭轮廓的覆盖局限，不增加缩颈鹭、鳽类或其他鹭科姿态变体；
- 全部正式抽象鸟图必须直接读取版本控制中已登记、可追溯的 PhyloPic SVG 几何作为真实外边界，并将服务器端 inline SVG 组合的身体分区与大色块裁切在来源轮廓内部；不能复制临时试稿的手绘外形来替代来源资产。`anatidae`／`ardeidae` 稳定 key、数据库结构和 MVP 范围均不改变；
- 已确认并用于 `anatidae` 的来源为 [PhyloPic `Anas platyrhynchos`](https://www.phylopic.org/images/1c044a03-f1ba-49fd-857e-f10aa8778e30/anas-platyrhynchos)，image UUID `1c044a03-f1ba-49fd-857e-f10aa8778e30`，作者 Andy Wilson，许可 CC0 1.0；`ardeidae` 为 [PhyloPic `Ardea goliath`](https://www.phylopic.org/images/b415196d-355a-408d-a73b-1567c87a1721/ardea-goliath)，image UUID `b415196d-355a-408d-a73b-1567c87a1721`，作者 Kai Caspar，许可 CC0 1.0；`compact_passerine` 应使用其既有登记来源 [PhyloPic `Cyanistes caeruleus`](https://www.phylopic.org/images/069c4833-e1ac-48e7-90d5-f7bd11000588/cyanistes-caeruleus)，image UUID `069c4833-e1ac-48e7-90d5-f7bd11000588`，作者 Wouter Koch，许可 CC0 1.0；
- `compact_passerine` 已使用上述已登记来源外边界；翼部只保留一个完整主色区域和一个连续、无深色描边的补充色区。翼带、翼斑、翼镜只决定该唯一内部区域的形状和位置，不再叠加“白色色块＋两条翼带”或其他独立小模块；当前固定小样使用 `wing_patch`；
- 阶段 8 不新增 Migration、数据库字段或表，也不引入 AI 生图、Canvas、远程绘图服务、自由绘图或直接拖拽修改鸟体曲线。

**暂定自动预览节奏**：选择项变化后即时请求，自由文字输入约 350ms 防抖；请求串行处理，只保留最新待发送状态。实现时需要验证快速连续输入、慢响应、失败响应和离开页面，不得让旧响应覆盖较新的表单状态。

**已确认的映射边界**：纵纹、横斑、斑点、杂斑、金属光泽、淡色羽缘、冠羽、叉尾和长尾只进入文字摘要，不在当前 MVP 的抽象 SVG 中绘制。重复斜线、点阵、密集羽片、冠羽／叉尾／长尾局部轮廓变体或其他精细纹理已经否决。色块型特征与补充色可以通过每个部位最多一个连续固定辅助区域表达；不得恢复第三层模块，具体区域路径和 SVG 坐标不构成产品决定。

**当前开发证据**：鸭科与鹭科换源、圆身小鸟登记来源外边界和三个代表性试稿已经完成并获得前述视觉确认。开发提交 `46c7900` 完成 17 个轮廓映射，`dce876e` 接入本地化摘要与详情／历史重新生成，`32ceee5` 接入 Turbo／Stimulus 渐进预览；当前代码已移除冠羽、叉尾和长尾局部 SVG 变体。开发任务报告 Rails 174 个测试／3827 个断言、System Test 19 个／380 个断言，均为 0 失败、0 错误、0 跳过；RuboCop 113 个文件／0 违规，Brakeman 0 警告，Zeitwerk、JavaScript 语法和差异检查通过，`db/schema.rb` 无变化。阶段 8 已完成工程实现、机械验证和用户验收；其余 14 个轮廓尚未逐一获得用户视觉确认，这是不阻塞阶段完成的已知覆盖缺口。
...
Turbo 和 Stimulus 已通过 importmap 加载。当前预览的主要互动仍由普通导航和 Stimulus 完成，尚未形成依赖服务器响应的 Turbo Frame 工作流；阶段 8 将在不改变现有正式保存事务的前提下补充该工作流。
...
1. 在 development 环境从注册页面提交真实表单；
2. 等待 Letter Opener 自动使用 Windows 默认浏览器打开邮件预览；如果没有弹出或页面被误关，访问当前开发备用收件箱 `/letter_opener`；
3. 由用户亲自点击邮件正文中的真实验证链接，不从日志、测试代码或其他入口直接打开 token URL；
4. 确认第一次 GET 只显示脱敏邮箱确认页，POST 二次确认后返回登录页并显示成功提示；
5. 确认此时没有自动登录，再手动登录并继续检查空观察历史与退出。

Letter Opener 只用于 development。测试环境继续使用 test adapter，生产环境的真实 SMTP、邮件供应商和持久化队列不在本地验收范围内。
...
| 轮廓选择 | 原型使用示例分组、名称、顺序和图片 | 正式页面已实现水面／水边／大鸟／小鸟两层入口、16 个具体轮廓和 1 个兜底选项，并使用本地 SVG | 正式清单见 `requirements.md` 第 7 节；17 张素材映射、裁切和显示效果是待视觉验收的实现事实，不得新增第三层或把科名保存为鉴定结果 |
| 次要颜色 | 高保真示例没有完整展开所有次要颜色状态 | 每个部位提供可选次要颜色，并允许取消 | 已确认适配：正式需求包含多个显著颜色 |
| 颜色、特征与位置列表 | 原型包含演示名称和有限示例 | 正式 Observation 表单已使用确认后的配置与稳定 key | 产品清单由 `requirements.md` 第 7 节拥有；阶段 7 已验收，尺寸、行列和移动端排版仍可作为 UI 实现细节继续优化 |
| 抽象鸟图 | 高保真基线展示概念性鸟图 | 17 个轮廓均已接入登记来源外边界和四分区映射，阶段 8 已完成用户验收 | 全部正式外边界来自登记的 PhyloPic SVG，内部由 inline SVG 组合；摘要专用特征不改变 SVG，色块型特征与补充色使用固定辅助区域。其余 14 个轮廓未逐一确认视觉效果是已知覆盖缺口 |
| 历史缩略图 | 原型展示不同鸟类轮廓 | 已从保存的结构化数据重新生成阶段 8 SVG | 生成后的缩略图不持久化；当前实现已纳入阶段 8 整体验收 |
| 账户表单 | 高保真基线覆盖账户及辅助流程 | 静态预览继续保留；阶段 6 正式账户页面已复用 B′ 布局和组件 | 已确认复用现有视觉组件；正式表单由服务器校验，阶段 6 的账户闭环与中日文可见反馈已通过用户验收 |
| 表单提交 | 原型点击可演示流程 | 静态预览仍标记 `data-static-preview="true"`；正式账户与 Observation 表单已有真实 action、Controller、CSRF 和服务器校验 | 已确认技术边界；Observation 使用聚合事务，编辑使用 expected revision；精确路径和错误微文案是可调整实现 |
| 语言切换 | 原型用于展示双语言页面 | 静态预览可用 `locale` query；正式页面把有效选择保存到加密语言 Cookie | 已确认行为：无 Cookie 时 `zh*` 映射中文，其余回退日文；内部 Cookie 键名仅是实现事实 |
...
| Turbo | 正式技术方向包含 Turbo | 编辑表单已通过 Stimulus 触发请求，并由 Turbo Frame 替换 SVG 与摘要预览 | 当前实现已通过机械验证，具体等待时间和调度细节仍可在用户验收后调整 |
| 桌面布局 | 高保真主要体现移动端应用表面 | 781px 以上显示居中的设备式页面容器 | 仅代码现状／暂定适配：保持 B′ 方向并便于桌面预览，不是独立设计系统决定 |
| 设置与修改密码 | 高保真基线包含设置和登录后修改密码页面 | 正式设置页显示真实邮箱、语言选择和真实退出；修改密码仍为延后提示，静态预览保留完整页面参考 | 当前阶段边界：真实 Session 与 locale 已接入，密码更新留后续账户辅助切片 |
| 错误与边界状态 | 原型只覆盖部分状态 | 静态预览保留查询参数效果；正式账户流程已由服务器决定字段错误、页面提示和结果状态 | 查询参数仅供预览；阶段 6 已通过用户验收，当前中日文微文案、提交后焦点和字段值保留细节仍可在后续实现中继续验证和调整 |
...
- 阶段 6 账户闭环与阶段 7 Observation 真实表单、持久化及当前并发冲突反馈已经通过机械验证和用户验收；
- 轮廓选择已有 17 张本地 PhyloPic CC0 SVG 及逐张许可记录；阶段 8 的 17 个动态部位着色映射、本地化摘要、详情／历史重新生成和渐进预览已经完成工程实现、机械验证和用户验收。其余 14 个轮廓尚未逐一获得用户视觉确认，但不阻塞阶段 8 完成；
- 静态预览页面本身仍不提交真实业务数据；正式账户与 Observation 页面已接入持久化，动态 SVG／摘要和业务预览也已接通；鸟种识别与密码辅助尚未完成；
- 当前 14 个页面已完成 320px、390px、桌面宽度、长日文、键盘顺序和短视口的集中回归；真实移动设备、软键盘和 Safari、Firefox 等多浏览器验证仍未完成；
- 正式页面语言选择已经保存到当前浏览器的加密 Cookie；静态预览仍可使用查询参数作为本地参考。
- 本地 TLS 已在相同配置、获批准的非沙箱环境下完成连接和测试对照；M1 另有认证基础验证记录，沙箱上下文仍可能失败。证据见 `backend-development-plan.md`，不再把更换 TLS 方案当作接入前置条件；本地连接成功不代表生产身份验证完成，M1 测试也不代替正式账户页面验收。

### 14.2 已排除、未确认或不在本文档中决定

- 轮廓选择素材与阶段 8 生成映射必须区分：17 张轮廓选择 SVG 已落地并记录 CC0 来源，17 套四分区映射也已实现。冠羽、叉尾和长尾只进入摘要，先前局部 SVG 变体已经否决并移除；其余 14 个轮廓尚未逐一确认视觉效果，但这不阻塞阶段 8 完成；
