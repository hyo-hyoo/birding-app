...
> 当前适用阶段：静态前端 14／14、阶段 6 账户闭环与阶段 7 Observation 核心闭环均已验收；阶段 8A 工程实现及细粒度特征映射边界已确认，阶段 8 批量扩展待实施<br>
...
- Observation 的两层轮廓选择、新建、非空历史、详情和重新编辑已经接入真实 Route、Controller、Model、三张 M3 表和服务器校验；动态 SVG／摘要、鸟种识别、密码重置和修改密码仍未接入；
...
- 正式提交只保存轮廓稳定 key，不把页面分段标题或科名提交为鸟类鉴定结果。分类元数据与轮廓素材引用分离；16 个具体轮廓、1 个兜底选项及颜色、特征、行动位置的正式清单以 `requirements.md` 第 7 节为准，不得继续采用静态预览旧示例；内部 key、具体 SVG 和许可记录由实施阶段落实；
...
- 17 个轮廓分别拥有头部、胸腹、翼、尾四个矢量分区，不使用一套通用粗略遮罩；冠羽、叉尾、长尾等结构特征只切换当前基础 SVG 的相应局部矢量表现，取消后恢复基础外形，不更换 `outline_key`、分类含义或来源记录；
...
**已确认的映射边界**：纵斑、横斑、斑点、杂斑、金属光泽和淡色羽缘只进入文字摘要，不在当前 MVP 的抽象 SVG 中绘制。重复斜线、点阵、密集羽片或其他精细纹理已经否决。色块型特征继续通过每个部位最多一个连续补充色区域表达是当前暂定方案；不得恢复第三层模块，具体区域路径和 SVG 坐标不构成产品决定。

**当前开发证据**：鸭科与鹭科换源实现及前一轮专项测试已完成，两者当前视觉方向已经用户接受。开发提交 `4955c3e` 又把圆身小鸟接到已登记的 `Cyanistes caeruleus` 源几何，移除两条翼带和多个独立小模块；相关 Model／Integration 测试 24 个／587 断言、完整 System Test 17 个／365 断言均为 0 失败、0 错误、0 跳过。开发任务重新生成并检查了 320px 日文与 390px 中文截图，三个小样均使用已登记来源外边界，内部补充色为连续大色块。以上证据支持阶段 8A 工程实现完成；后续按新确认的细粒度特征边界批量实施其余轮廓。
...
| 抽象鸟图 | 高保真基线展示概念性鸟图 | 鸭科、鹭科已更换水面／站立 PhyloPic 源几何并获用户视觉接受；圆身小鸟也已接入登记来源外边界和单一翼内区域，阶段 8A 工程实现完成 | 三个试稿确认大色块、每部位最多一个连续补充色区等视觉语法；全部正式外边界来自登记的 PhyloPic SVG，内部由 inline SVG 组合。细粒度特征只进摘要，结构特征只局部改变当前基础轮廓；旧实现和临时试稿路径都不是正式资产 |
| 历史缩略图 | 原型展示不同鸟类轮廓 | 当前使用手写 SVG variant；阶段 8 将从已保存结构化数据重新生成 | 当前 variant 不是正式业务图片；生成后的缩略图不持久化 |
...
- 轮廓选择当前已有 17 张本地 PhyloPic CC0 SVG 及逐张许可记录；阶段 8A 三个代表性试稿的视觉语法已经确认，鸭科水面鸭与鹭科站立鹭的新来源和实际视觉方向也已确认，圆身小鸟已完成登记来源外边界、单一翼内区域、测试和代表性尺寸截图检查。其余 14 个轮廓的动态部位着色、摘要与历史缩略图整体仍未完成；
- 静态预览页面本身仍不提交真实业务数据；正式账户与 Observation 页面已接入持久化，但动态 SVG／摘要、鸟种识别、密码辅助及其业务 Turbo 响应链路尚未完成；
...
- 轮廓选择素材与阶段 8 生成映射必须区分：17 张轮廓选择 SVG 已落地并记录 CC0 来源；17 套四分区映射、结构特征局部变化和单一连续补充色区域已经确认。鸭科、鹭科的新来源及实际视觉方向已经验收，圆身小鸟代表性实现与相关测试已经完成；其余 14 个轮廓的后续矢量绘制仍未完成；
