...
> 当前适用阶段：静态前端 14／14、阶段 6 账户闭环与阶段 7 Observation 核心闭环均已验收；阶段 8 SVG、摘要与渐进预览准备实施<br>
...
- 补充色绘制到每个轮廓、每个部位固定的辅助区域。部位自由文字只进入本地化摘要，不参与图形映射；
- 映射使用版本控制配置。具体文件名、Ruby 类名和对象拆分是可调整的工程实现，不能反向改变正式 UI 或产品规则；
- 先实现 `anatidae`、`ardeidae`、`compact_passerine` 三个代表性小样，用户验收视觉效果后再扩展剩余 14 个轮廓；
...
| 抽象鸟图 | 高保真基线展示概念性鸟图 | 当前 ERB Partial 仍输出固定概念 SVG；阶段 8 将改为 17 套四分区服务器生成映射 | 当前 Partial 是 observed-only；先验收 `anatidae`、`ardeidae`、`compact_passerine` 三个小样，再扩展其余轮廓 |
...
- 轮廓选择当前已有 17 张本地 PhyloPic CC0 SVG 及逐张许可记录；动态部位着色鸟图、摘要和历史缩略图尚未实现，阶段 8 先以三个代表性小样验证实际视觉效果；
...
- 轮廓选择素材与阶段 8 生成映射必须区分：17 张轮廓选择 SVG 已落地并记录 CC0 来源；17 套四分区映射、结构特征局部变化和固定补充色区域已经确认，但具体矢量绘制效果尚未实现，三个代表性小样仍待用户验收；
