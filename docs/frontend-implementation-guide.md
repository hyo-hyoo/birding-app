...
> 当前适用阶段：静态前端 14／14、阶段 6 账户闭环与阶段 7 Observation 核心闭环均已验收；阶段 8A 视觉语法与鸭科／鹭科来源方向已确认，换源效果待验收<br>
...
- `anatidae`、`ardeidae`、`compact_passerine` 三个代表性试稿已经验证并确认上述视觉语法。用户确认 `anatidae` 正式改用水面鸭、`ardeidae` 正式改用站立鹭，并继续采用来源可追溯、许可明确的 PhyloPic 轮廓。开发任务当前采用的来源分别为：[PhyloPic `Anas platyrhynchos`](https://www.phylopic.org/images/1c044a03-f1ba-49fd-857e-f10aa8778e30/anas-platyrhynchos)，image UUID `1c044a03-f1ba-49fd-857e-f10aa8778e30`，作者 Andy Wilson，许可 CC0 1.0；[PhyloPic `Ardea goliath`](https://www.phylopic.org/images/b415196d-355a-408d-a73b-1567c87a1721/ardea-goliath)，image UUID `b415196d-355a-408d-a73b-1567c87a1721`，作者 Kai Caspar，许可 CC0 1.0；
- 渲染器必须直接读取版本控制中的可信 PhyloPic SVG 几何，并将大色块裁切在正式源轮廓内部；不能复制临时试稿的手绘外形来替代来源资产。`anatidae`／`ardeidae` 稳定 key、数据库结构和 MVP 范围均不改变；
...
**仍待确认的映射边界**：纵斑、横斑、斑点、杂斑、金属光泽和淡色羽缘等细粒度特征，哪些只进入摘要、哪些可在不增加视觉层级的前提下转化为一个大面积色区。`requirements.md` 6.7 仍要求抽象图表达已记录的预设花纹或特征；如果要让某些标签只进入摘要，必须先由用户确认该产品行为变化。未确认前不得自行省略，也不得沿用旧密集斜线、重复纹理或多个独立小模块。

**当前开发证据**：开发任务报告两个本地 SVG 源文件和 observation options 来源元数据已经替换，渲染器已直接读取可信源几何且内部补充色无深色描边；相关普通测试 35 个／1817 断言、阶段 8A 专项 System Test 2 个／15 断言均为 0 失败、0 错误、0 跳过。以上修改尚未提交或推送，且换源后的实际视觉效果仍待用户验收，因此不能把阶段 8A 标记完成。
...
| 抽象鸟图 | 高保真基线展示概念性鸟图 | 开发工作区已为鸭科、鹭科更换水面／站立 PhyloPic 源几何，并让渲染器直接读取可信 SVG；阶段 8 整体仍未完成 | 三个试稿确认大色块、每部位最多一个连续补充色区等视觉语法；两项来源方向已确认并通过专项测试，但改动未提交、未推送且实际效果待用户验收，旧实现和临时试稿路径都不是正式资产 |
...
- 轮廓选择当前已有 17 张本地 PhyloPic CC0 SVG 及逐张许可记录；阶段 8A 三个代表性试稿的视觉语法已经确认，鸭科水面鸭与鹭科站立鹭的新来源也已确认并在开发工作区换源。当前换源效果尚未经过用户验收，动态部位着色鸟图、摘要和历史缩略图仍未完成；
...
- 轮廓选择素材与阶段 8 生成映射必须区分：17 张轮廓选择 SVG 已落地并记录 CC0 来源；17 套四分区映射、结构特征局部变化和单一连续补充色区域已经确认。鸭科、鹭科的新来源已经确认，但换源后的视觉效果与剩余矢量绘制仍未完成验收；
