...
> 当前适用阶段：静态前端 14／14 已验收；后端阶段 6 的 M1 认证基础已验证，正式页面接入仍待 M2<br>
...
- 后端已有 M1 User／Session 模型、两表迁移与认证基础，但尚未接入正式路由或 ApplicationController；这些页面仍不读写真实账户或观察数据；
- 刷新页面会重置浏览器内的交互状态；
...
M1 的认证请求测试通过独立测试 Controller、路由和 JSON 响应验证数据库 Session 与 Cookie，不属于正式页面或产品 API。后续 M2 接入应复用现有前端布局和组件，而不是把测试入口接给用户；实施与验证证据见 [backend-development-plan.md](backend-development-plan.md) 12.5 节。
...
- 当前预览页面没有真实业务表单提交、认证接入、业务持久化或服务器校验；M1 认证基础的存在不代表这些页面已经接通，也没有完成业务 Turbo 响应链路；
