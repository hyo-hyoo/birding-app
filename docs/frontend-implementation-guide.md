...
> 当前适用阶段：静态前端 14／14 与阶段 6 账户闭环已验收；阶段 7 Observation 核心闭环已完成工程实施和机械验证，等待用户验收<br>
> 最近核对日期：2026-09-05<br>
...
**当前阶段 7 实现事实**：正式 Observation 页面已经按上述边界接入真实持久化。编辑提交携带 expected revision，服务器在父记录行锁和整体事务中比较版本；当前冲突响应为 422 重新渲染、保留输入，并提示返回最新详情后重新编辑。该冲突表现仍待用户验收，可以调整文案和交互，但不得取消防止旧页面覆盖新数据的规则。
...
| 颜色、特征与位置列表 | 原型包含演示名称和有限示例 | 正式 Observation 表单已使用确认后的配置与稳定 key | 产品清单由 `requirements.md` 第 7 节拥有；当前尺寸、行列和移动端排版仍待阶段 7 用户验收 |
...
- 阶段 6 账户闭环已经通过用户验收；阶段 7 Observation 真实表单、持久化与并发冲突反馈已经实现并通过机械验证，用户可见流程仍待验收；
- 轮廓选择当前已有 17 张本地 PhyloPic CC0 SVG 及逐张许可记录；其映射、裁切和显示效果仍待视觉验收，动态部位着色鸟图和历史缩略图仍不是最终生成效果；
...
- 轮廓素材与抽象鸟图生成细节：17 张轮廓选择 SVG 已作为当前实现落地并记录 CC0 来源，但视觉验收尚未完成；服务器生成 inline SVG 的高层路线已经确认，动态部位映射和抽象图生成细节仍未实现，不在本文档中决定；
