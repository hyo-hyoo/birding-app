...
**已确认业务与风险边界**：`deliver_later` 的切换点是“新令牌保存且发送任务实际入队成功”，入队不等于送达。开发进程退出可能丢失内存队列任务，该风险已由用户接受；入队后失败不恢复旧链接。具体事务、Job 协作和故障处理协议见 [database-design.md](database-design.md) 第 14.3 节，属于授权范围内的工程设计，尚须在 M2 实施时通过故障注入与并发测试验证，不是已经实现的功能。
